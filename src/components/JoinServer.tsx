...
export function JoinServer() {
  ...
  return (
    ...
    >
      ...
      <div 
          ref={joinRef}
          className="max-w-3xl mx-auto glass-card p-8 md:p-12 reveal-anim reveal-up"
        >
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold mb-3">{t.join.title}</h2>
          ...
          <div className="mb-8 animate-on-scroll fade-up">
            ...
            <div className="flex">
              <input
                ...
                value={serverAddress}
                readOnly
                className="flex-1 p-3 bg-background border border-border rounded-l-lg focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <button
                onClick={copyToClipboard}
                className="bg-primary text-primary-foreground px-4 rounded-r-lg flex items-center gap-2 hover:bg-primary/90 transition-colors"
              >
                {copied ? (
                  <>
                    <Check className="h-4 w-4" /> {t.join.copied}
                  </>
                ) : (
                  <>
                    <Copy className="h-4 w-4" /> {t.join.copy}
                  </>
                )}
              </button>
            </div>
          ...
          <div 
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                <span className="text-white font-medium">Server Lobby</span>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-bold mb-4 reveal-anim reveal-left">Quick Steps to Join</h3>
              <ol ref={stepsRef} className="space-y-3 mb-6">
                <li className="flex gap-3 animate-on-scroll fade-left">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground grid place-items-center">1</span>
                  <span>{t.java.step1}</span>
                </li>
                <li className="flex gap-3 animate-on-scroll fade-left">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground grid place-items-center">2</span>
                  <span>{t.java.step2}</span>
                </li>
                <li className="flex gap-3 animate-on-scroll fade-left">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground grid place-items-center">3</span>
                  <span>{t.java.step3}</span>
                </li>
                <li className="flex gap-3 animate-on-scroll fade-left">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground grid place-items-center">4</span>
                  <span>{t.java.step4}</span>
                </li>
                <li className="flex gap-3 animate-on-scroll fade-left">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground grid place-items-center">5</span>
      ...
      <div className="absolute right-[10%] bottom-[20%] w-12 h-12 opacity-70 parallax" data-speed="-0.15">
